...
def basic_trend_following(pd_dataframe):
    ...
    pd_dataframe = pd_dataframe.assign(Fast10DayEMA=pd.Series(ema_fast_values, index=pd_dataframe.index))
    pd_dataframe = pd_dataframe.assign(Slow40DayEMA=pd.Series(ema_slow_values, index=pd_dataframe.index))
    pd_dataframe = pd_dataframe.assign(APO=pd.Series(apo_values, index=pd_dataframe.index))
    pd_dataframe = pd_dataframe.assign(Trades=pd.Series(orders, index=pd_dataframe.index))
    pd_dataframe = pd_dataframe.assign(Position=pd.Series(positions, index=pd_dataframe.index))
    pd_dataframe = pd_dataframe.assign(Pnl=pd.Series(pnls, index=pd_dataframe.index))
