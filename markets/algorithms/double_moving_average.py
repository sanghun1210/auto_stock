...
def double_moving_average(financial_data, short_window, long_window):
    ...
    signals['signal'] = 0.0
    signals['short_mavg'] = financial_data['trade_price'].\
        rolling(window=short_window,
                min_periods=1, center=False).mean()
    signals['long_mavg'] = financial_data['trade_price'].\
...
def plot_double_moving_average(ts, pd_dataframe):
    fig = plt.figure()
    ax1 = fig.add_subplot(111, ylabel='Google price in $')
    pd_dataframe["trade_price"].plot(ax=ax1, color='g', lw=.5)
    ts["short_mavg"].plot(ax=ax1, color='r', lw=2.)
    ts["long_mavg"].plot(ax=ax1, color='b', lw=2.)

    ax1.plot(ts.loc[ts.orders== 1.0].index,
            pd_dataframe["trade_price"][ts.orders == 1.0],
